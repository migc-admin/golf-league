...
export const DEFAULT_STANDINGS_CONFIG = {
...
// ─── Cross-event standings computation ─────────────────────────────────────────
// Reused by both a league's "Season" tab (LeagueHome) and a trip's "Standings"
// tab (TripHome) — both simply repoint `leagueId` at the relevant leagues.id.
export function useSeasonStandings() {
  const [standings,        setStandings]        = useState([])
  const [sideGameStats,    setSideGameStats]    = useState([])
  const [standingsLoading, setStandingsLoading] = useState(false)

  async function loadStandings(leagueId, standingsConfig) {
    setStandingsLoading(true)
    const cfg = { ...DEFAULT_STANDINGS_CONFIG, ...(standingsConfig ?? {}) }
  ...
  return { standings, sideGameStats, standingsLoading, loadStandings }
}
...
export function EventCard({ event, leagueSlug, orgSlug, muted = false }) {
  ...
  return (
    <div className={`bg-white border rounded-xl shadow-sm p-5 flex flex-col gap-2 ${muted ? 'opacity-70' : 'hover:shadow-md transition-shadow'}`}>
      ...
      {event.slug && (
        <Link
          to={orgSlug ? `/${orgSlug}/${leagueSlug}/${event.slug}/event` : `/${leagueSlug}/${event.slug}`}
...
// ─── Money List ───────────────────────────────────────────────────────────────
export function MoneyList({ standings, events }) {
...
// ─── Side Game Table ──────────────────────────────────────────────────────────
export function SideGameTable({ stats, hasSkins, hasCTP, hasLD }) {
  return (
    <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
      <table className="w-full text-sm">
        <thead>
